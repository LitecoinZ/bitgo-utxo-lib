...
function Transaction (network = networks.bitcoin) {
  ...
  if (coins.isZcash(network) || coins.isLitecoinZ(network)) {
    // ZCash version >= 2
    this.joinsplits = []
...
Transaction.fromBuffer = function (buffer, network = networks.bitcoin, __noStrict) {
  ...
  if (marker === Transaction.ADVANCED_TRANSACTION_MARKER &&
      ...
      !coins.isZcash(network) && !coins.isLitecoinZ(network)) {
    offset += 2
    hasWitnesses = true
  }
...
Transaction.prototype.isSaplingCompatible = function () {
  return (coins.isZcash(this.network) || coins.isLitecoinZ(this.network)) && this.version >= Transaction.ZCASH_SAPLING_VERSION
}

Transaction.prototype.isOverwinterCompatible = function () {
  return (coins.isZcash(this.network) || coins.isLitecoinZ(this.network)) && this.version >= Transaction.ZCASH_OVERWINTER_VERSION
}

Transaction.prototype.supportsJoinSplits = function () {
  return (coins.isZcash(this.network) || coins.isLitecoinZ(this.network)) && this.version >= Transaction.ZCASH_JOINSPLITS_SUPPORT_VERSION
}
...
Transaction.prototype.zcashTransactionByteLength = function () {
  if (!coins.isZcash(this.network) && !coins.isLitecoinZ(this.network)) {
